...
build = import_abspy(
    "models", 
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "../VMamba/classification/"),
)
...
def mmengine_flop_count(model: nn.Module = None, input_shape = (3, 224, 224), show_table=False, show_arch=False, _get_model_complexity_info=False):
    ...
    # from mmcv.cnn import get_model_complexity_info as mm_get_model_complexity_info
    ...
    def get_model_complexity_info(
        ...
        flop_handler = FlopAnalyzer(model, construct_input(input_shape)).set_op_handle(**supported_ops)
    ...
    model.eval()
    # flops, params = mm_get_model_complexity_info(
...
def fvcore_flop_count(model: nn.Module, inputs=None, input_shape=(3, 224, 224), show_table=False, show_arch=False, supported_ops=supported_ops, verbose=True, occ=False):
    ...
    # print(input_shape)
    if inputs is None and occ==False:
    ...
    if occ == True:
        inputs = construct_input(input_shape)
    model.eval()
...
def main0():
    ...
    # modes = ["vssmaav1"]
    modes = ["vssmaav1_backbone"]
    # modes = ["resnet"]
    _build = import_abspy("models", f"{os.path.dirname(__file__)}/../VMamba/classification")
    ...
    def _count(model, size, modify_attn=False, **kwargs):
        ...
        if type(size) != list:
            ...
            fvcore_flop_count(model, input_shape=(3,size[0], size[1]), show_table=True)

    # vssm ta6: install selective_scan
    if "vssma6" in modes:
        print("vssm ta6 ================================", flush=True)
        import triton, mamba_ssm, selective_scan_cuda_oflex
        _model = import_abspy("vmamba", os.path.join(f"{os.path.dirname(__file__)}","../classification/models"))
    ...
    if "vssmaav1" in modes:
        ...
        for size in [[512,1408]]: # [224, 384, 512, 640, 768, 1024]:
            _count(taav1, size)
    
    if "vssmaav1_backbone" in modes:
        print("vssm taav1 ================================", flush=True)
        _model = import_abspy("vmamba", os.path.join(f"{os.path.dirname(__file__)}","../VMamba/classification/models"))
        taav1 = partial(_model.Backbone_VSSM, dims=96, depths=[2,2,5,2], ssm_d_state=1, ssm_dt_rank="auto", ssm_ratio=2.0, ssm_conv=3, ssm_conv_bias=False, forward_type="v05_noz", mlp_ratio=4.0, downsample_version="v3", patchembed_version="v2", norm_layer="ln2d")
        for size in [[256,704]]: # [224, 384, 512, 640, 768, 1024]:
            _count(taav1, size)
    ...
    if "resnet" in modes:
        ...
        for size in [224]: # [224, 384, 512, 640, 768, 1024]:
            _count(tiny, size)
...
def build_occ_model(config, input_shape):
    cfg = Config.fromfile(config)
    ...
    # mmengine_flop_count(model, input_shape=(3, 256,704))
    # fvcore_flop_count(model, input_shape=input_shape, show_table=True, occ=True)
    fvcore_flop_count(model.img_backbone, input_shape=input_shape, show_table=True, occ=False)

def count_model(model, input_shape):
    fvcore_flop_count(model, input_shape, show_table=True)

if __name__ == '__main__':
    # check_mmengine_equals_fvcore()
    # main_check_ops()
    # breakpoint()
    cfg_path = 'projects/configs/flashocc/flashocc-r50_wo_pretrain.py'
    cfg_path = 'projects/configs/flashocc/flashocc-r50.py'
    # cfg_path = 'projects/configs/mambaocc/mambaocc_tiny_wo_pretrain.py'
    # cfg_path = 'projects/configs/mambaocc/mambaocc-stbase-4d-stereo-512x1408_4x4_1e-2_w_imgpretrain.py'
    shape_input = [3,256,704]
    # shape_input = [3,512,1408]
    # [3,512,1408]
    build_occ_model(cfg_path,shape_input)
    # projects/configs/flashocc/flashocc-r50_wo_pretrain.py
    # projects/configs/mambaocc/mambaocc_tiny_wo_pretrain.py
    sys.exit()
    main0()
    sys.exit()
    breakpoint()
