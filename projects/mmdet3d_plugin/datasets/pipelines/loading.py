...
class PointToEgoSystem(object):
    ...
    def __call__(self, results):
        ...
        points_lidar = results['points']    # [n, 5]
        imgs, sensor2egos, ego2globals, intrins = results['img_inputs'][:4]
        post_rots, post_trans, bda = results['img_inputs'][4:]
        ...
        # # lidar --> lidar_ego --> global --> cam_ego --> cam
        # lidar2camego = torch.inverse(camego2global).matmul(
        #     lidarego2global.matmul(lidar2lidarego))
        # lidar2img = cam2img.matmul(lidar2cam)

        points_ego = points_lidar.tensor[:, :3].matmul(
            lidar2lidarego[:3, :3].T) + lidar2lidarego[:3, 3].unsqueeze(0)     # (N_points, 3)  
        points = bda.matmul(points_ego.T).T

        # points = bda.view(B, 1, 1, 1, 1, 3,
        #                   3).matmul(points_ego.unsqueeze(-1)).squeeze(-1)
        # import ipdb;ipdb.set_trace()
