...
from projects.mmdet3d_plugin.models.backbones.vmamba import MM_VSSM
    ...
    def extract_stereo_ref_feat(self, x):
        ...
        if isinstance(self.img_backbone, ResNet):
            ...
            for i, layer_name in enumerate(self.img_backbone.res_layers):
                ...
                x = res_layer(x)
                return x
        elif isinstance(self.img_backbone, MM_VSSM):
            ...
            for i, layer in enumerate(self.img_backbone.layers):
                out, x = layer_forward(layer, x) # (B, H, W, C) torch.Size([12, 96, 128, 352])
                return out
        else:
            x = self.img_backbone.patch_embed(x)
