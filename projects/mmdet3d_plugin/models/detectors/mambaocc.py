...
from mmdet3d.models import DETECTORS, build_model
...
class MambaOcc(BEVDet):
    def __init__(self,
                 occ_head=None,
                 upsample=False,
                 teacher_info=None,
                 **kwargs):
        super(MambaOcc, self).__init__(**kwargs)
    ...
    def bev_encoder(self, x):
        ...
        Returns:
            x: (B, C', 2*Dy, 2*Dx)
        """
        x_bev_encoder = self.img_bev_encoder_backbone(x)
        x_bev_neck = self.img_bev_encoder_neck(x_bev_encoder)
        return [x_bev_neck, x_bev_encoder]
    ...
    def simple_test(self,
        ...
        # pts_feats: None
        # depth: (B*N_views, D, fH, fW)
        if self.pts_voxel_layer != None:
            points = points.data[0]
            points[0] = points[0].to(img[0].device)
    ...
    def forward_dummy(self,
        ...
        # pts_feats: None
        # depth: (B*N_views, D, fH, fW)
        if points != None and 'img_inputs' in points.keys() and img_inputs == None:
            img_inputs = points['img_inputs']
            points=None
        img_feats, pts_feats, depth = self.extract_feat(
            points, img_inputs=img_inputs, img_metas=img_metas, **kwargs)
        occ_bev_feature = img_feats[0]
...
@DETECTORS.register_module()
class MambaStereo4DOCC(BEVStereo4D):
    def __init__(self,
                 occ_head=None,
                 upsample=False,
                 **kwargs):
        super(MambaStereo4DOCC, self).__init__(**kwargs)
        self.occ_head = build_head(occ_head)
        self.pts_bbox_head = None
        self.upsample = upsample
